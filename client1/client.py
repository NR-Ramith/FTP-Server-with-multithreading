...
def upld(file_name):
    ...
    try:
        ...
        s.send(bytes(file_name, encoding='ascii'))
...
def conn(details):
    ...
    passwd = details.split()[1]
    print("Sending server request...")
    try:
        s.connect((socket.gethostname(), TCP_PORT))
        print("Connection successful")
        try:
            #Make user authentication request
            s.send(b"USER")
        except:
            print("Connection unsuccessful. Make sure the server is online.")
        try:
            # Wait for server acknowledgement then send user detail
            # Wait for server ok
            s.recv(BUFFER_SIZE)
            #Send username size and username
            s.send(struct.pack("h", sys.getsizeof(user_name)))
            s.send(bytes(user_name, encoding='ascii'))
            # Wait for server ok then send file size
            s.recv(BUFFER_SIZE)
            #Send password size and password
            s.send(struct.pack("h", sys.getsizeof(passwd)))
            s.send(bytes(passwd, encoding='ascii'))
            # Wait for server ok then send file size
            s.recv(BUFFER_SIZE)
        except:
            print("Error sending user detail")
        try:
            auth = struct.unpack("i", s.recv(4))[0]
            if(auth==1):
                print(user_name,"authentication successful.")
                return 0
            else:
                print("Name or password invalid. Connection is being closed.")
                return 1
        except:
            print("Error in receiving authentication status.") 
    except:
        print("Connection unsuccessful. Make sure the server is online.")


def start_client():
    print ("\n\nWelcome to the FTP client.\n\nCall one of the following functions:\nCONN user_name passwd : Connect to server\nUPLD file_path : Upload file\nLIST           : List files\nDWLD file_path : Download file\nDELF file_path : Delete file\n\nQUIT           : Exit")

    while True:
        # Listen for a command
        prompt =input("\nEnter a command: ")
        if prompt[:4].upper() == "CONN":
            flag=conn(prompt[5:].upper())
            if(flag):
                quit()
                break
